...
> andor2 = make_pblock "andor2" ["i", "j"] ["o0","o1"] "  assign o0 = i & j;\n  assign o1 = i | j;\n"

andor2n, as implemented below is somewhat of a hack compared to "proper" way of
doing it which would involve reversing inputs of structure produced by
xbintree_arb but latter seems to assume all block outputs are connections to
parents (only inputs can be "horizontal" connections) which is a shortcoming
which prevents connection reversal using rec_revio (which also may have bugs).

> andor2n levels = foldr andor2n_1 pblk_id (reverse [1..levels])

> andor2n_1 level tree_blk =
>   make_block ((name_ andor2)++"n_"++(show level)) (["i"]++(wires0 "j" level)) (wires0 "o" (2^level))
>                ((inst_block andor2 (["i"]++["j"++(show (level-1))]++(wires0 "_t" 2))):
>                 (map (\i->inst_block tree_blk ((wires "_t" i 1)++(wires0 "j" (level-1))++
>                                                (wires "o" (i*(2^(level-1))) (2^(level-1))))) [0,1]))

Above andor yields the "greater than or equal to" function (fig. 3 in
r.pdf). To obtain required lte function, need to bitwise reverse the
outputs. Also, the input is hardwired to 1.

> lte levels = let size = 2^levels
>              in make_block ("lte_"++(show levels)) (["i"]++(wires0 "j" levels)) (wires0 "o" size)
>                 [inst_block (andor2n levels) (["one"]++(wires0 "j" levels)++(wires0 "u" size)),inst_block one ["one"],
>                  inst_block (permute "revbits" reverse_bits size) ((wires0 "u" size)++(reverse (wires0 "v" size))),
>                  inst_block (chain' gate_and2 gate_and2 ["i0"] [] size) (["i"]++(wires0 "v" size)++(wires0 "o" size))]

> gte levels = let size = 2^levels
>              in make_block ("gte_"++(show levels)) (wires0 "j" levels) (wires0 "o" size)
...
Output is 1 if input is lte/gte given constant. Based on FPL 2008
paper. Alternative algo. would be to use constant coefficient adder/subtracter.

Implementation is somewhat different as foldl is used and (unlike chain)
single module is generated. Can a chain variant be utilized? Or can chain be
replaced by foldr/foldl?

> lte_const1 (const',blks) i = (div const' 2,((inst_block gate_invert ["i"++(show i),"i"++(show i)++"_"])):
>                                             ((inst_block (if mod const' 2 == 1 then gate_or2 else gate_and2)
>                                              ["t"++(show i),"i"++(show i)++"_","t"++(show (i+1))]):blks))

> lte_const0 size const = foldl lte_const1 (const,[inst_block one ["t0"]]) [0..(size-1)]

> lte_const size const = make_block ("lte_"++(show size)++"_"++(show const)) (wires0 "i" size) ["o"]
>                        ((inst_block pblk_id ["t"++(show size),"o"]):(snd (lte_const0 size const)))

> gte_const1 (const',blks) i = (div const' 2,((inst_block (if mod const' 2 == 0 then gate_or2 else gate_and2)
>                                              ["t"++(show i),"i"++(show i),"t"++(show (i+1))]):blks))

> gte_const0 size const = foldl gte_const1 (const,[inst_block one ["t0"]]) [0..(size-1)]

> gte_const size const = make_block ("gte_"++(show size)++"_"++(show const)) (wires0 "i" size) ["o"]
>                        ((inst_block pblk_id ["t"++(show size),"o"]):(snd (gte_const0 size const)))
